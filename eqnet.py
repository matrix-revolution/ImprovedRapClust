...
class RapClust_Code(object):

    def buildNetFile(self, sampdirs, netfile, cutoff, auxDir, writecomponents=False):
        ...
        estCount = None
        quant = None
        for sffile, eqfile in list(zip(sffiles, eqfiles)):
        ...
        rapObj = RapClust_Code()
        with open(netfile, 'w') as ofile:
            rapObj.writeEdgeList(weightDict, tnames, ofile, G)
    ...
    def writeEdgeList(self, weightDict, tnames, ofile, G):
    ...
    def writePajek(self, netfile, weightDict, tnamesFilt, relabel, ofile):
        with open(netfile, 'w') as ofile:
    ...
    def readEqClass(self, eqfile, eqCollection):
        with open(eqfile) as ifile:
            numTran = int(ifile.readline().rstrip())
            numEq = int(ifile.readline().rstrip())
    ...
    def getCountsFromEquiv(self, eqCollection):
        countDict = {}
    ...
    def flattenClusters(self, infile, outfile):
    ...
    def filterGraph(self, expDict, netfile, ofile, auxDir):
        ...
        for cond in conditions:
            ...
            for sampNum, sampPath in expDict[cond].items():
                if cond not in eqClasses:
                    eqClasses[cond] = rapObj.EquivCollection()
                eqPath = os.path.sep.join([sampPath, auxDir, "eq_classes.txt"])
                rapObj.readEqClass(eqPath, eqClasses[cond])

        ambigCounts = {cond : rapObj.getCountsFromEquiv(eqClasses[cond]) for cond in conditions}
...
def main():
    ...
    rapObj.filterGraph(expDict, netfile, ofile, auxDir)
